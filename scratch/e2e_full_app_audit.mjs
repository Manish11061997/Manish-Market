...
(async () => {
  ...
  page.on('requestfailed', req => {
    ...
    if (!url.includes('google-analytics') && !url.includes('doubleclick')) {
      recordIssue('Medium', 'Network Failure', `Failed HTTP request: ${req.method()} ${url}`, req.failure()?.errorText || 'Unknown');
    }
  });
  ...
  try {
    ...
    const tabs = [
      { name: 'Watchlist / Terminal', selector: 'text=Watchlist', testAction: async () => {
        const count = await page.locator('.stock-card, [data-testid="stock-card"]').count();
      ...
      { name: 'F&O Trading Hub', selector: 'text=F&O, button:has-text("F&O"), nav >> text=F&O', testAction: async () => {
      ...
      { name: 'Paper Trading Hub', selector: 'text=Paper Trading, button:has-text("Paper Trading")', testAction: async () => {
        await page.waitForTimeout(1500);
      ...
      { name: 'Stock Screener', selector: 'text=Screener, button:has-text("Screener")', testAction: async () => {
        await page.waitForTimeout(1500);
      ...
      { name: 'Backtester', selector: 'text=Backtest, button:has-text("Backtester"), button:has-text("Backtest")', testAction: async () => {
        await page.waitForTimeout(1500);
        const runBtn = page.locator('button:has-text("Run Backtest"), button:has-text("Execute")').first();
      ...
      { name: 'IPO Hub', selector: 'text=IPO, button:has-text("IPO")', testAction: async () => {
        await page.waitForTimeout(1500);
      }},
      { name: 'AI Analysis Engine', selector: 'text=AI Analysis, button:has-text("AI Analysis")', testAction: async () => {
        await page.waitForTimeout(1500);
      }},
      { name: 'Daily Advisory Hub', selector: 'text=Advisory, button:has-text("Advisory")', testAction: async () => {
        await page.waitForTimeout(1500);
      }}
    ];
    ...
    const mainNav = page.locator('text=Watchlist, text=Markets').first();
